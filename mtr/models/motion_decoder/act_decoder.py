...
class ACTDecoder(nn.Module):
    ...
    def build_motion_query(self, d_model, use_place_holder=False):
        ...
        if use_place_holder:
            ...
            # visualize intention points
            # import matplotlib.pyplot as plt
            # for cur_type in self.object_type:
            #     cur_intention_points = intention_points_dict[cur_type]
            #     plt.scatter(cur_intention_points[:, 0], cur_intention_points[:, 1], s=1)


            intention_points = {}
            for cur_type in self.object_type:
                cur_intention_points = intention_points_dict[cur_type]/8
    ...
    def get_decoder_loss(self, tb_pre_tag=''):
        ...
        for layer_idx in range(self.num_decoder_layers):
            ...
            pred_trajs_gmm, pred_z, pred_heading = pred_trajs[:, :, :, 0:5], pred_trajs[:, :, :, 5],pred_trajs[:, :, :, 6]
            ...
            pred_heading = pred_heading[torch.arange(num_center_objects), center_gt_positive_idx]
            loss_reg_heading = F.l1_loss(pred_heading, center_gt_trajs[:, :, 3], reduction='none')
            loss_reg_heading  = (loss_reg_heading * center_gt_trajs_mask[:, :]).sum(dim=-1).sum(dim=-1)

            pred_z = pred_z[torch.arange(num_center_objects), center_gt_positive_idx]
            loss_reg_z = F.l1_loss(pred_z, center_gt_trajs[:, :, 2], reduction='none')
            loss_reg_z = (loss_reg_z * center_gt_trajs_mask[:, :]).sum(dim=-1).sum(dim=-1)
            ...
            tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_reg_heading'] = loss_reg_heading.mean().item() * weight_heading
            tb_dict[f'{tb_pre_tag}loss_layer{layer_idx}_reg_z'] = loss_reg_z.mean().item() * weight_z
